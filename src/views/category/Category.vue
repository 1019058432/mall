...
<script>
  ...
  import {POP, NEW, SELL, TOP_DISTANCE} from "common/Mixin/const";
  import {getCategory, getSubcategory, getCategoryDetail} from "network/category";
  import {itemListenerMixin, backTopMixin, tabControlMixin} from "common/Mixin/mixin";
  export default {
    name: "Category",
    components: {
      CategoryMenu,
    ...
    activated() {
      //一进入组件就滚动到离开时保存的位置
      this.$refs.scroll && this.$refs.scroll.scrollTop(0, this.saveY, 10);
      //refresh
      this.$refs.scroll && this.$refs.scroll.finishAll();
    },
    deactivated() {
      //保存离开时的位置
